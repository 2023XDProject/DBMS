...
public:
    ...
    //修改字段
    bool alterFieldEntity(vector<CFieldEntity> &cfe,string& fieldName,int& type,int& param,string& constraint);
    //重命名字段
    bool RenameFieldEntity(vector<CFieldEntity> &cfe,string& oldName,string& newName);
    //删除字段
    bool DropFieldEntity(vector<CFieldEntity> &cfe,string& a);
    //修改表信息
    bool alterTableEntity(vector<CTableEntity> &cte,string name,int fieldNum);
private:
